#!/usr/bin/env node
/**
 * The 6ix · apply 0009_the_6ix_intake.sql to Supabase.
 *
 * Usage: node --env-file=.env.local scripts/apply-the-6ix-intake-migration.mjs
 *
...
const sqlPath = path.resolve("supabase/migrations/0009_the_6ix_intake.sql");
...
try {
  ...
  const { rows: structure } = await client.query(
    `select column_name, data_type, is_nullable, column_default
     ...
     where table_schema = 'public' and table_name = 'the_6ix_intake'
     order by ordinal_position`
  );
  console.log("\nthe_6ix_intake columns:");
  ...
  const { rows: counts } = await client.query(
    "select count(*)::int as c from public.the_6ix_intake"
  );
  console.log(`\nrows in the_6ix_intake: ${counts[0].c}`);
} finally {
  await client.end();
}
