#!/usr/bin/env node
/**
 * AOC · apply 0010_aoc_waitlist.sql to Supabase.
 *
 * Usage: node --env-file=.env.local scripts/apply-aoc-waitlist-migration.mjs
 *
...
const sqlPath = path.resolve("supabase/migrations/0010_aoc_waitlist.sql");
...
try {
  ...
  const { rows: counts } = await client.query(
    "select count(*)::int as c from public.aoc_waitlist"
  );
  console.log(`\nrows in aoc_waitlist: ${counts[0].c}`);
} finally {
  await client.end();
}
