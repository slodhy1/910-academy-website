...
// Admin notification for the /aoc/livedemo demo-booking funnel. Fires on every
// completion (booked or texting). Template is INLINED (no emails/*.html file), so
// no next.config.ts outputFileTracingIncludes entry is required for this route.
...
export async function sendAocLivedemoNotify(p: LivedemoNotifyParams): Promise<SendResult> {
  ...
  const subject =
    p.type === "booked"
      ? "[AOC Demo] QUALIFIED — booked"
      : `[AOC Demo] TEXT — ${(p.fullName || "—").trim()} ${(p.phone || "").trim()}`.trim();
  ...
  const contactRows =
    p.type === "texting"
      ? row("Name", p.fullName || "—") + row("Phone", p.phone || "—")
      : `<tr><td colspan="2" style="padding:6px 0;color:#666;">Booking time &amp; contact details arrive via Calendly's own notification.</td></tr>`;
  ...
  const html =
    ...
    `<div style="color:#fff;font-size:20px;font-weight:800;margin-top:6px;">${p.type === "booked" ? "Qualified lead — demo booked" : "New text-capture lead"}</div>` +
    `</td></tr>` +
    `<tr><td style="padding:24px 28px;">` +
    `<table role="presentation" cellpadding="0" cellspacing="0" style="width:100%;font-size:14px;line-height:1.5;">` +
    row("Result", p.qualified ? "QUALIFIED" : "Not qualified") +
    row("Outcome", p.type === "booked" ? "Booked (Calendly)" : "Texting capture") +
    contactRows +
    `<tr><td colspan="2" style="border-top:1px solid #eee;padding-top:12px;"></td></tr>` +
    row("Q1 · Time in business", p.q1) +
    row("Q2 · Monthly earnings", p.q2) +
    row("Q3 · Focus area", p.q3) +
